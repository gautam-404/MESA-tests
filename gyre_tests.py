...
def evo_star(args):
    name, mass, metallicity, v_surf_init, gyre_input, logging, parallel, cpu_this_process, produce_track = args
    trace = None
    ...
    if gyre:
        try:
            ...
            proj.runGyre(gyre_in=gyre_input, files=profiles, data_format="GYRE", 
...
if __name__ == "__main__":
    gyre_inputs = glob.glob("./gyre_inputs/*")
    gyre_types = [gyre_in.split('/')[-1].split('.')[0] for gyre_in in gyre_inputs]
    # print(gyre_types)
    # exit()
    ...
    combinations = list(itertools.product(M_sample, Z_sample, V_sample, gyre_inputs))
    ...
    names = []
    i = 0
    for m, z, v, gyre_in in combinations:
        M.append(m)
        Z.append(z)
        V.append(v)
        gyre_inputs.append(gyre_in)
        names.append(f"test/m{m}_z{z}_v{v}_diff_{gyre_types[i]}")
        i += 1

    length = len(gyre_inputs)
    ...
    if parallel:
        ...
        with progress.Progress(*helper.progress_columns()) as progressbar:
            ...
            with Pool(n_procs, initializer=helper.mute) as pool:
                args = zip(names, M, Z, V, gyre_inputs, repeat(True), repeat(True), repeat(cpu_per_process), repeat(produce_track))
                for _ in enumerate(pool.imap_unordered(evo_star, args)):
                    progressbar.advance(task)
    else:
        os.environ["OMP_NUM_THREADS"] = '8'
        for i in range(len(gyre_inputs)):
            evo_star((names[i], M[i], Z[i], V[i], gyre_inputs[i], True, True, cpu_per_process, produce_track))
