...
def evo_star(test_profiles_folder, name, mass, metallicity, v_surf_init, 
                logging, parallel, cpu_this_process, num_steps_to_relax_rotation=100):
    # if v_surf_init == 0:
    #     num_steps_to_relax_rotation = 100
    # else:
    #     num_steps_to_relax_rotation /= v_surf_init
    produce_track = True
...
if __name__ == "__main__":
    ...
    for num_steps_to_relax_rotation in [100, 250, 500, 1000]:
        num_steps_to_relax_rotation = 100
        for M in [1.4]:
            for Z in [0.008]: 
                ...
                with progress.Progress(*helper.progress_columns()) as progressbar:
                    ...
                    with Pool(n_procs, initializer=helper.mute) as pool:
                        args = zip(repeat(test_profiles_folder), [f"tests_here/track{i}" for i in range(1, length+1)], repeat(M), repeat(Z), V,
                                    repeat(True), repeat(True), repeat(cpu_per_process), repeat(num_steps_to_relax_rotation))
                        for _ in pool.istarmap(evo_star, args):
                            progressbar.advance(task)
        plots.hist_plots(f'tests_here/test_profiles_M1.4_Z0.008_steps{num_steps_to_relax_rotation}', 'surf_avg_v_rot', unit='km/s', logarithmic=False)
        plt.savefig(f'surf_avg_v_rot_steps{num_steps_to_relax_rotation}.png', dpi=300)

        logs_dir = f'tests_here/test_profiles_M1.4_Z0.008_steps{num_steps_to_relax_rotation}'
        v, freqs, hists, profs = plots.get_data(logs_dir)
        os.mkdir(f"{logs_dir}/echelle_{num_steps_to_relax_rotation}")
        for i in range(len(v)):
            plots.plot_echelle(logs_dir,i)
            plt.savefig(f"{logs_dir}/echelle_{num_steps_to_relax_rotation}/echelle_{i}.png", dpi=300)
            plt.close()
